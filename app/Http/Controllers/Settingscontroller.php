...
class SettingsController extends Controller
{
 ...
 public function fcm(Request $request)
	...
	public function setorder_charge(Request $request)
    {
        $title = "Order Charge Setting";
		...
		$order_charge = DB::table('minimum_maximum_order_value')
                ->first();   
		return view('admin.settings.order_charge',compact("admin_email","admin",'title','logo','order_charge'));
    }
 
    public function updateorder_charge(Request $request)
    {
        $min = $request->min_value;
        $max = $request->max_charge;
        $this->validate(
            $request,
                [
                    'min_value'=>'required',
                ...
                [
                    'min_value.required' =>'Enter delivery charge',
                    'max_charge.required'=>'Enter Minimum Cart Value'
                ]
        );
		...
		if($check){
			$update = DB::table('minimum_maximum_order_value')
					->update(['min_value'=> $min,
					'max_value'=>$max]);
		}else{
			  $update = DB::table('minimum_maximum_order_value')
					->insert(['min_value'=> $min,
					 'max_value'=>$max]);
		}if($update){
			return redirect()->back()->withSuccess('Delivery Charge Updated Successfully');
		}else{
